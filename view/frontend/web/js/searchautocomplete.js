            ...
            ajaxSearch: function (e) {
                ...
                xhr = $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $( ".cp-footer" ).text(config.resultFooter);
                        if(data.suggestions > 0) {
                            $('.cp-search-autocomplete ul li').each(function() {
                                if (!$.trim($(this).text())) {
                                    $(this).remove();
                                }
                            });
                            $('.cp-search-autocomplete ul li').css('cursor', 'pointer');
                            $('.cp-search-autocomplete ul li').click(function(){
                                $('#search').val($(this).find('.qs-option-name').text());
                                $('#search_mini_form').submit();
                            });
                        } else {
                            $('.cp-search-autocomplete ul li').css('cursor', 'default');
                        }
                    },
                    complete: function(){
